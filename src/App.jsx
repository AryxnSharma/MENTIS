...
const Seal = () => (
  <svg width="84" height="84" viewBox="0 0 90 90" fill="none">
    <circle cx="45" cy="45" r="42" stroke="#C9A84C" strokeWidth="1.5" strokeDasharray="4 2"/>
    <circle cx="45" cy="45" r="36" stroke="#C9A84C" strokeWidth="0.5"/>
    <text x="45" y="37" textAnchor="middle" fontSize="17" fill="#C9A84C" fontFamily="serif">🎭</text>
    <text x="45" y="51" textAnchor="middle" fontSize="6" fill="#C9A84C" fontFamily="serif" letterSpacing="2">COMEDY</text>
    <text x="45" y="60" textAnchor="middle" fontSize="6" fill="#C9A84C" fontFamily="serif" letterSpacing="2">& LAW</text>
  </svg>
);

const P = ({ children }) => (
  <p style={{ fontSize: "clamp(0.92rem,2.5vw,0.98rem)", color: "#b8ad94", lineHeight: 1.78, margin: "0.65rem 0", fontFamily: "'Crimson Pro',serif" }}>{children}</p>
);
const H2 = ({ children, accent }) => (
  <h2 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: "clamp(1.15rem,3.5vw,1.4rem)", color: "#EDE8D8", margin: "2rem 0 0.55rem", lineHeight: 1.2, borderBottom: `1px solid ${accent||"rgba(237,232,216,0.1)"}`, paddingBottom: "0.45rem" }}>{children}</h2>
);
const H3 = ({ children, accent }) => (
  <h3 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 600, fontSize: "0.98rem", color: accent||"#C9A84C", margin: "1.4rem 0 0.35rem", letterSpacing: "0.01em" }}>{children}</h3>
);
const B = ({ children }) => <strong style={{ color: "#EDE8D8", fontWeight: 600 }}>{children}</strong>;
const Em = ({ children }) => <em style={{ color: "#C9A84C", fontStyle: "italic" }}>{children}</em>;

const Divider = ({ accent }) => (
  <div style={{ display: "flex", alignItems: "center", gap: "1rem", margin: "2rem 0" }}>
    <div style={{ flex: 1, height: "1px", background: "rgba(237,232,216,0.07)" }} />
    <span style={{ color: accent||"#C9A84C", fontSize: "0.75rem", opacity: 0.5 }}>§</span>
    <div style={{ flex: 1, height: "1px", background: "rgba(237,232,216,0.07)" }} />
  </div>
);

const Rule = ({ icon, label, children }) => (
  <div style={{ display: "flex", gap: "0.7rem", margin: "0.6rem 0", alignItems: "flex-start" }}>
    <span style={{ fontSize: "0.9rem", flexShrink: 0, marginTop: "2px" }}>{icon}</span>
    <div style={{ fontSize: "0.89rem", color: "#b8ad94", lineHeight: 1.65, fontFamily: "'Crimson Pro',serif" }}>
      {label && <strong style={{ color: "#EDE8D8" }}>{label} — </strong>}
      {children}
    </div>
  </div>
);
...
const Step = ({ num, title, children }) => (
  <div style={{ display: "flex", gap: "0.9rem", margin: "1.1rem 0" }}>
    <div style={{ width: "30px", height: "30px", borderRadius: "50%", border: "1px solid #C9A84C", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontFamily: "'Playfair Display',serif", color: "#C9A84C", fontSize: "0.82rem", fontWeight: 700, marginTop: "2px" }}>{num}</div>
    <div>
      {title && <div style={{ fontSize: "0.84rem", fontWeight: 700, color: "#EDE8D8", marginBottom: "0.25rem", fontFamily: "'Playfair Display',serif" }}>{title}</div>}
      <div style={{ fontSize: "0.89rem", color: "#b8ad94", lineHeight: 1.65, fontFamily: "'Crimson Pro',serif" }}>{children}</div>
    </div>
  </div>
);

const LawRef = ({ children }) => (
  <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: "0.76rem", color: "#C9A84C", background: "rgba(201,168,76,0.1)", padding: "1px 5px", borderRadius: "3px", whiteSpace: "nowrap" }}>{children}</span>
);

const Warning = ({ children }) => (
  <div style={{ background: "rgba(179,27,27,0.07)", border: "1px solid rgba(179,27,27,0.3)", borderLeft: "3px solid #B31B1B", borderRadius: "4px", padding: "0.9rem 1.1rem", margin: "1.3rem 0" }}>
    <div style={{ fontSize: "0.6rem", fontFamily: "'JetBrains Mono',monospace", color: "#B31B1B", letterSpacing: "0.1em", marginBottom: "0.35rem", fontWeight: 700 }}>⚠ CAUTION</div>
    <div style={{ fontSize: "0.89rem", color: "#d4c9a8", lineHeight: 1.65, fontFamily: "'Crimson Pro',serif" }}>{children}</div>
  </div>
);

const Defense = ({ children }) => (
  <div style={{ background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.22)", borderLeft: "3px solid #C9A84C", borderRadius: "4px", padding: "0.9rem 1.1rem", margin: "1.3rem 0" }}>
    <div style={{ fontSize: "0.6rem", fontFamily: "'JetBrains Mono',monospace", color: "#C9A84C", letterSpacing: "0.1em", marginBottom: "0.35rem", fontWeight: 700 }}>✓ YOUR DEFENSE</div>
    <div style={{ fontSize: "0.89rem", color: "#d4c9a8", lineHeight: 1.65, fontFamily: "'Crimson Pro',serif" }}>{children}</div>
  </div>
);

const Verdict = ({ children }) => (
  <div style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(237,232,216,0.1)", borderRadius: "6px", padding: "1rem 1.2rem", margin: "1.5rem 0" }}>
    ...
    <div style={{ fontSize: "0.93rem", color: "#EDE8D8", lineHeight: 1.72, fontStyle: "italic", fontFamily: "'Crimson Pro',serif" }}>{children}</div>
  </div>
);

// ─── COMEDY-SPECIFIC COMPONENTS ─────────────────────────────────────────────
const Formula = ({ children }) => (
  <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(237,232,216,0.08)", borderRadius: "6px", padding: "0.9rem 1.1rem", margin: "1.2rem 0", textAlign: "center" }}>
    ...
    <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: "clamp(0.72rem,2vw,0.82rem)", color: "#C9A84C", lineHeight: 1.7 }}>{children}</div>
  </div>
);

const Exercise = ({ label, children }) => (
  <div style={{ background: "rgba(46,196,182,0.05)", border: "1px solid rgba(46,196,182,0.2)", borderLeft: "3px solid #2EC4B6", borderRadius: "4px", padding: "0.9rem 1.1rem", margin: "1.4rem 0" }}>
    <div style={{ fontSize: "0.6rem", fontFamily: "'JetBrains Mono',monospace", color: "#2EC4B6", letterSpacing: "0.1em", marginBottom: "0.35rem", fontWeight: 700 }}>✏ {label||"EXERCISE"}</div>
    <div style={{ fontSize: "0.89rem", color: "#b8ad94", lineHeight: 1.65, fontFamily: "'Crimson Pro',serif" }}>{children}</div>
  </div>
);

const KeyNote = ({ children }) => (
  <div style={{ background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.18)", borderRadius: "6px", padding: "0.9rem 1.1rem", margin: "1.3rem 0" }}>
    <div style={{ fontSize: "0.6rem", fontFamily: "'JetBrains Mono',monospace", color: "#C9A84C", letterSpacing: "0.1em", marginBottom: "0.35rem" }}>★ KEY CONCEPT</div>
    <div style={{ fontSize: "0.92rem", color: "#EDE8D8", lineHeight: 1.7, fontStyle: "italic", fontFamily: "'Crimson Pro',serif" }}>{children}</div>
  </div>
);

const Persona = ({ emoji, name, desc, examples }) => (
  <div style={{ borderLeft: "2px solid rgba(201,168,76,0.25)", paddingLeft: "1rem", margin: "0.9rem 0" }}>
    <div style={{ fontSize: "0.88rem", color: "#EDE8D8", fontWeight: 600, fontFamily: "'Playfair Display',serif", marginBottom: "0.2rem" }}>{emoji} {name}</div>
    <div style={{ fontSize: "0.85rem", color: "#b8ad94", lineHeight: 1.6, fontFamily: "'Crimson Pro',serif" }}>{desc}</div>
...
const TableComp = ({ headers, rows }) => (
  <div style={{ overflowX: "auto", margin: "1.1rem 0", WebkitOverflowScrolling: "touch" }}>
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82rem", fontFamily: "'Crimson Pro',serif", minWidth: "320px" }}>
      <thead>
        <tr>{headers.map((h,i) => <th key={i} style={{ textAlign:"left", padding:"0.55rem 0.75rem", background:"rgba(201,168,76,0.09)", color:"#C9A84C", fontFamily:"'JetBrains Mono',monospace", fontSize:"0.6rem", letterSpacing:"0.08em", fontWeight:700, borderBottom:"1px solid rgba(201,168,76,0.18)", whiteSpace:"nowrap" }}>{h}</th>)}</tr>
      </thead>
      <tbody>{rows.map((row,i) => <tr key={i} style={{ borderBottom:"1px solid rgba(237,232,216,0.05)" }}>{row.map((cell,j) => <td key={j} style={{ padding:"0.55rem 0.75rem", color: j===0?"#EDE8D8":"#b8ad94", lineHeight:1.5, verticalAlign:"top" }}>{cell}</td>)}</tr>)}</tbody>
    </table>
  </div>
);

const SectionRef = ({ id, label, number, title, sub, accent, children, innerRef }) => (
  <div ref={innerRef} id={id} style={{ scrollMarginTop: "52px" }}>
    <div style={{ padding: "2.8rem 0 0.5rem", borderTop: "1px solid rgba(237,232,216,0.05)", marginTop: "1.5rem" }}>
      <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: "0.58rem", color: accent||"#B31B1B", letterSpacing: "0.16em", marginBottom: "0.35rem" }}>{label}</div>
      <h1 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 800, fontSize: "clamp(1.5rem,5.5vw,2.1rem)", color: "#EDE8D8", lineHeight: 1.12, marginBottom: "0.35rem", letterSpacing: "-0.01em" }}>{title}</h1>
      <p style={{ fontSize: "0.83rem", color: "#555", fontStyle: "italic", fontFamily: "'Crimson Pro',serif" }}>{sub}</p>
    </div>
...
export default function CompleteRoastBook() {
  ...
  return (
    <div style={{ background: "#0D1117", minHeight: "100vh", display: "flex", flexDirection: "column", fontFamily: "'Crimson Pro',serif", overflowX: "hidden" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,600;0,700;0,800;1,400;1,600&family=Crimson+Pro:ital,wght@0,300;0,400;0,600;1,300;1,400;1,600&family=JetBrains+Mono:wght@400;700&display=swap');
        *{box-sizing:border-box;margin:0;padding:0;}
        ::-webkit-scrollbar{width:3px;}
        ::-webkit-scrollbar-track{background:#0D1117;}
        ::-webkit-scrollbar-thumb{background:#222;border-radius:2px;}
        .toc-row:hover{background:rgba(201,168,76,0.07)!important;cursor:pointer;}
        .nav-btn:hover{opacity:0.75;}
      `}</style>

      {/* ── Sticky Header ── */}
      <div style={{ position:"sticky", top:0, zIndex:200, background: scrolled?"rgba(13,17,23,0.96)":"#0D1117", borderBottom: scrolled?"1px solid rgba(237,232,216,0.06)":"none", backdropFilter:"blur(10px)", padding:"0.55rem 1rem", display:"flex", alignItems:"center", justifyContent:"space-between", transition:"all 0.2s", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", gap:"0.55rem", minWidth:0 }}>
          <span style={{ color:"#C9A84C", fontSize:"0.85rem", flexShrink:0 }}>🎭</span>
          <div style={{ minWidth:0 }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"0.78rem", color:"#EDE8D8", fontWeight:700, lineHeight:1, whiteSpace:"nowrap" }}>The Complete Roast Handbook</div>
        ...
        <button className="nav-btn" onClick={() => setTocOpen(!tocOpen)} style={{ background: tocOpen?"rgba(201,168,76,0.12)":"transparent", border:"1px solid rgba(201,168,76,0.22)", borderRadius:"5px", color:"#C9A84C", padding:"0.3rem 0.65rem", fontSize:"0.68rem", fontFamily:"'JetBrains Mono',monospace", cursor:"pointer", flexShrink:0, letterSpacing:"0.04em" }}>
          {tocOpen ? "✕" : "≡ CONTENTS"}
        </button>
      </div>
      ...
      {/* ── TOC Overlay ── */}
      {tocOpen && (
        <div style={{ position:"sticky", top:"42px", zIndex:199, background:"#111520", borderBottom:"1px solid rgba(237,232,216,0.05)", maxHeight:"70vh", overflowY:"auto" }}>
          {/* Part I Header */}
          <div style={{ padding:"0.65rem 1rem 0.25rem", borderBottom:"1px solid rgba(237,232,216,0.04)" }}>
            <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.58rem", color:"#2EC4B6", letterSpacing:"0.14em" }}>PART I</div>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"0.85rem", color:"#888", fontWeight:600 }}>Comedy Craft</div>
          </div>
          {SECTIONS.filter(s => s.part===1).map(s => (
            <div key={s.id} className="toc-row" onClick={() => scrollTo(s.id)} style={{ padding:"0.5rem 1rem 0.5rem 1.4rem", display:"flex", gap:"0.75rem", alignItems:"center", background: activeId===s.id?"rgba(46,196,182,0.05)":"transparent" }}>
              <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.58rem", color:"#2EC4B6", width:"68px", flexShrink:0 }}>{s.label}</span>
              <div>
                <div style={{ fontSize:"0.8rem", color: activeId===s.id?"#2EC4B6":"#EDE8D8", fontFamily:"'Playfair Display',serif", lineHeight:1.2 }}>{s.title}</div>
                <div style={{ fontSize:"0.6rem", color:"#444", marginTop:"1px" }}>{s.sub}</div>
              </div>
            </div>
          ))}
          {/* Part II Header */}
          <div style={{ padding:"0.65rem 1rem 0.25rem", borderTop:"1px solid rgba(237,232,216,0.06)", borderBottom:"1px solid rgba(237,232,216,0.04)", marginTop:"0.25rem" }}>
            <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.58rem", color:"#B31B1B", letterSpacing:"0.14em" }}>PART II</div>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"0.85rem", color:"#888", fontWeight:600 }}>The Roast & The Law</div>
          </div>
          {SECTIONS.filter(s => s.part===2).map(s => (
            <div key={s.id} className="toc-row" onClick={() => scrollTo(s.id)} style={{ padding:"0.5rem 1rem 0.5rem 1.4rem", display:"flex", gap:"0.75rem", alignItems:"center", background: activeId===s.id?"rgba(179,27,27,0.06)":"transparent" }}>
              <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.58rem", color:"#B31B1B", width:"68px", flexShrink:0 }}>{s.label}</span>
              <div>
                <div style={{ fontSize:"0.8rem", color: activeId===s.id?"#C9A84C":"#EDE8D8", fontFamily:"'Playfair Display',serif", lineHeight:1.2 }}>{s.title}</div>
                <div style={{ fontSize:"0.6rem", color:"#444", marginTop:"1px" }}>{s.sub}</div>
              </div>
            </div>
          ))}
      ...
      <div ref={scrollEl} style={{ flex:1, overflowY:"auto", overflowX:"hidden" }}>
        <div style={{ maxWidth:"680px", margin:"0 auto", padding:"0 1.1rem 5rem" }}>
          ...
          {/* ════ COVER ════ */}
          <div style={{ textAlign:"center", padding:"3.5rem 0 2.5rem", borderBottom:"1px solid rgba(237,232,216,0.06)" }}>
            <Seal />
            ...
            <h1 style={{ fontFamily:"'Playfair Display',serif", fontWeight:800, fontSize:"clamp(2rem,8.5vw,3.2rem)", color:"#EDE8D8", lineHeight:1.08, letterSpacing:"-0.02em", marginBottom:"0.5rem" }}>
              Comedy Craft<br/><em style={{ color:"#C9A84C" }}>&</em> Creator Law
            </h1>
            <p style={{ fontSize:"clamp(0.92rem,2.8vw,1.05rem)", color:"#555", fontFamily:"'Crimson Pro',serif", fontStyle:"italic", maxWidth:"300px", margin:"0 auto 1.8rem", lineHeight:1.6 }}>
              From writing your first roast to surviving your first legal notice
            </p>
            <div style={{ display:"flex", gap:"0.75rem", justifyContent:"center", flexWrap:"wrap" }}>
              <div style={{ background:"rgba(46,196,182,0.08)", border:"1px solid rgba(46,196,182,0.2)", borderRadius:"4px", padding:"0.35rem 0.9rem" }}>
                <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.6rem", color:"#2EC4B6", letterSpacing:"0.06em" }}>PART I: 5 UNITS · 14 LESSONS · 20+ DRILLS</span>
              </div>
              <div style={{ background:"rgba(179,27,27,0.08)", border:"1px solid rgba(179,27,27,0.2)", borderRadius:"4px", padding:"0.35rem 0.9rem" }}>
                <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.6rem", color:"#B31B1B", letterSpacing:"0.06em" }}>PART II: 6 LEGAL CHAPTERS · NOT LEGAL ADVICE</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ padding:"2.5rem 0 1rem", textAlign:"center" }}>
            <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.6rem", color:"#2EC4B6", letterSpacing:"0.22em", marginBottom:"0.4rem" }}>PART I</div>
            <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:800, fontSize:"clamp(1.4rem,5vw,1.9rem)", color:"#EDE8D8" }}>Comedy Craft</div>
            <div style={{ fontFamily:"'Crimson Pro',serif", fontStyle:"italic", color:"#555", fontSize:"0.9rem", marginTop:"0.4rem" }}>A complete course in joke writing</div>
            <div style={{ height:"1px", background:"linear-gradient(90deg,transparent,rgba(46,196,182,0.3),transparent)", margin:"1.5rem 0 0" }} />
          </div>

          {/* ════ UNIT 0: FOUNDATIONS ════ */}
          <SectionRef id="p1-u0" label="FOUNDATIONS" title="The Architecture of a Joke" sub="Before you write anything, understand what a joke actually is" accent="#2EC4B6" innerRef={el => refs.current["p1-u0"] = el}>

            <H2 accent="rgba(46,196,182,0.3)">Lesson 1 — What Makes Something Actually Funny</H2>
            ...
            <H3 accent="#2EC4B6">The Rule of Three</H3>
            ...
            <H3 accent="#2EC4B6">Specificity Beats Vagueness Every Time</H3>
            ...
            <Divider accent="#2EC4B6" />

            <H2 accent="rgba(46,196,182,0.3)">Lesson 2 — The Anatomy of a Punchline</H2>
            ...
            <H3 accent="#2EC4B6">The Button Word</H3>
            ...
            <H3 accent="#2EC4B6">The Callback</H3>
            ...
            <H3 accent="#2EC4B6">The Topper</H3>
          ...
          {/* ════ UNIT I: ROAST ════ */}
          <SectionRef id="p1-u1" label="UNIT I" title="The Art of the Roast" sub="How to destroy someone out of love" accent="#2EC4B6" innerRef={el => refs.current["p1-u1"] = el}>

            <H2 accent="rgba(46,196,182,0.3)">Lesson 1 — Roasting vs. Bullying: The Critical Line</H2>
            ...
            <H3 accent="#2EC4B6">The Affection Test</H3>
            ...
            <H3 accent="#2EC4B6">The Self-Deprecation Trick</H3>
            ...
            <H2 accent="rgba(46,196,182,0.3)">Lesson 2 — Roast Structure: Opening, Escalation & The Closer</H2>
            ...
            <H3 accent="#2EC4B6">Act 1 — The Soft Opener</H3>
            ...
            <H3 accent="#2EC4B6">Act 2 — The Escalation</H3>
            ...
            <H3 accent="#2EC4B6">Act 3 — The Character Indictment</H3>
            ...
            <H3 accent="#2EC4B6">Act 4 — The Reversal</H3>
            ...
            <H3 accent="#2EC4B6">Your Tone Toolkit</H3>
            ...
            <H2 accent="rgba(46,196,182,0.3)">Lesson 3 — The Callback Roast: Building a Mythology</H2>
            ...
            <H3 accent="#2EC4B6">The Callback Loop</H3>
            ...
            <H3 accent="#2EC4B6">The Myth Arc</H3>
          ...
          {/* ════ UNIT II: STAND-UP ════ */}
          <SectionRef id="p1-u2" label="UNIT II" title="Stand-Up & Entertainment Writing" sub="Finding your voice and building material that holds a room" accent="#2EC4B6" innerRef={el => refs.current["p1-u2"] = el}>

            <H2 accent="rgba(46,196,182,0.3)">Lesson 1 — Finding Your Voice: The Comic Persona</H2>
            ...
            <H3 accent="#2EC4B6">The Four Classic Personas</H3>
            ...
            <H3 accent="#2EC4B6">The Voice Test</H3>
            ...
            <H2 accent="rgba(46,196,182,0.3)">Lesson 2 — The Bit: Writing Extended Observational Comedy</H2>
            ...
            <H3 accent="#2EC4B6">Step 1: The Premise — "Funny Because It's True"</H3>
            ...
            <H3 accent="#2EC4B6">Step 2: Explore Every Angle</H3>
            ...
            <H3 accent="#2EC4B6">Step 3: Heighten Progressively</H3>
            ...
            <H3 accent="#2EC4B6">Step 4: The Absurdist Peak</H3>
            ...
            <H3 accent="#2EC4B6">Step 5: The Tag</H3>
            ...
            <H2 accent="rgba(46,196,182,0.3)">Lesson 3 — Hosting, Emceeing & Writers' Rooms</H2>
            ...
            <H3 accent="#2EC4B6">Late Night Joke Structure</H3>
            ...
            <H3 accent="#2EC4B6">The Self-Aware Host</H3>
            ...
            <H3 accent="#2EC4B6">Writers' Room Dynamics</H3>
          ...
          {/* ════ UNIT III: CINEMATIC ════ */}
          <SectionRef id="p1-u3" label="UNIT III" title="Cinematic & Script Comedy" sub="Jokes that live inside characters, not above them" accent="#2EC4B6" innerRef={el => refs.current["p1-u3"] = el}>

            <H2 accent="rgba(46,196,182,0.3)">Lesson 1 — Comedy in Dialogue</H2>
            ...
            <H3 accent="#2EC4B6">The Comedy of Mismatched Want</H3>
            ...
            <H3 accent="#2EC4B6">The Reveal-and-React</H3>
            ...
            <H3 accent="#2EC4B6">Subtext in Comedy Dialogue</H3>
            ...
            <H3 accent="#2EC4B6">The Callback in Scripts</H3>
            ...
            <H2 accent="rgba(46,196,182,0.3)">Lesson 2 — Comedy Scene Construction</H2>
            ...
            <H3 accent="#2EC4B6">Beat 1 — Establish the Normal World</H3>
            ...
            <H3 accent="#2EC4B6">Beat 2 — Introduce the Inciting Absurdity</H3>
            ...
            <H3 accent="#2EC4B6">Beat 3 — Escalate Relentlessly</H3>
            ...
            <H3 accent="#2EC4B6">Beat 4 — The Button</H3>
            ...
            <H2 accent="rgba(46,196,182,0.3)">Lesson 3 — Character Comedy: The Flaw is the Engine</H2>
            ...
            <H3 accent="#2EC4B6">The Classic Comedy Flaws</H3>
            ...
            <H3 accent="#2EC4B6">The Comedy Character Worksheet</H3>
            ...
            <H2 accent="rgba(46,196,182,0.3)">Lesson 4 — Writing Satire That Actually Says Something</H2>
            ...
            <H3 accent="#2EC4B6">The Three Types</H3>
            ...
            <H3 accent="#2EC4B6">The Satirist's Trap: Preachiness</H3>
          ...
          {/* ════ UNIT IV: ADVANCED ════ */}
          <SectionRef id="p1-u4" label="UNIT IV" title="Advanced Techniques" sub="The moves that separate good writers from dangerous ones" accent="#2EC4B6" innerRef={el => refs.current["p1-u4"] = el}>

            <H2 accent="rgba(46,196,182,0.3)">Lesson 1 — The Misdirection Stack</H2>
            ...
            <H3 accent="#2EC4B6">The Double Subversion</H3>
            ...
            <H3 accent="#2EC4B6">The Brick Wall</H3>
            ...
            <H3 accent="#2EC4B6">The Compounding Topper</H3>
            ...
            <H2 accent="rgba(46,196,182,0.3)">Lesson 2 — Writing Dark Comedy Without Crashing</H2>
            ...
            <H3 accent="#2EC4B6">The Four Distance Tools</H3>
            ...
            <H2 accent="rgba(46,196,182,0.3)">Lesson 3 — The Editing Process: Kill Your Darlings</H2>
            ...
            <H3 accent="#2EC4B6">The Read-Aloud Rule</H3>
            ...
            <H3 accent="#2EC4B6">The Word Economy Test</H3>
            ...
            <H3 accent="#2EC4B6">The Punchline Position Test</H3>
            ...
            <H3 accent="#2EC4B6">The Three-Draft System</H3>
          ...
          <div style={{ padding:"3rem 0 1rem", textAlign:"center", marginTop:"1rem" }}>
            <div style={{ height:"1px", background:"linear-gradient(90deg,transparent,rgba(179,27,27,0.3),transparent)", marginBottom:"2rem" }} />
            <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.6rem", color:"#B31B1B", letterSpacing:"0.22em", marginBottom:"0.4rem" }}>PART II</div>
            <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:800, fontSize:"clamp(1.4rem,5vw,1.9rem)", color:"#EDE8D8" }}>The Roast & The Law</div>
            <div style={{ fontFamily:"'Crimson Pro',serif", fontStyle:"italic", color:"#555", fontSize:"0.9rem", marginTop:"0.4rem" }}>Legal risks, defenses, and what to do when things go wrong</div>
            <div style={{ background:"rgba(179,27,27,0.08)", border:"1px solid rgba(179,27,27,0.2)", borderRadius:"4px", padding:"0.35rem 0.9rem", display:"inline-block", marginTop:"1rem" }}>
              <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.58rem", color:"#B31B1B", letterSpacing:"0.06em" }}>EDUCATIONAL ONLY · NOT LEGAL ADVICE · CONSULT AN ADVOCATE</span>
            </div>
          </div>

          {/* ════ CH I: WHAT CAN GET YOU SUED ════ */}
          <SectionRef id="p2-ch1" label="CHAPTER I" title="What Can Get You Sued" sub="Know the charges before you step into the ring" accent="#B31B1B" innerRef={el => refs.current["p2-ch1"] = el}>
          ...
          {/* ════ CH II: LEGAL NOTICE ════ */}
          <SectionRef id="p2-ch2" label="CHAPTER II" title="Legal Notice Received" sub="Step-by-step survival when the letter arrives" accent="#B31B1B" innerRef={el => refs.current["p2-ch2"] = el}>
          ...
          <SectionRef id="p2-ch3" label="CHAPTER III" title="Fake Copyright Strikes" sub="How to fight back and how to win" accent="#B31B1B" innerRef={el => refs.current["p2-ch3"] = el}>
          ...
          <SectionRef id="p2-ch4" label="CHAPTER IV" title="Public vs. Private Figures" sub="The single distinction that changes everything" accent="#B31B1B" innerRef={el => refs.current["p2-ch4"] = el}>
          ...
          {/* ════ CH V: DOS AND DON'TS ════ */}
          <SectionRef id="p2-ch5" label="CHAPTER V" title="Dos & Don'ts" sub="The roast creator's legal code" accent="#B31B1B" innerRef={el => refs.current["p2-ch5"] = el}>
          ...
          {/* ════ CH VI: DISCLAIMER ════ */}
          <SectionRef id="p2-ch6" label="CHAPTER VI" title="The Disclaimer" sub="One line that does more legal work than you'd believe" accent="#B31B1B" innerRef={el => refs.current["p2-ch6"] = el}>
            ...
            <div style={{ background:"rgba(201,168,76,0.05)", border:"1px solid rgba(201,168,76,0.18)", borderRadius:"6px", padding:"1.1rem 1.3rem", margin:"1.1rem 0" }}>
              <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.58rem", color:"#C9A84C", letterSpacing:"0.08em", marginBottom:"0.6rem" }}>STANDARD VERSION</div>
              <p style={{ fontFamily:"'Crimson Pro',serif", fontSize:"1rem", color:"#EDE8D8", lineHeight:1.7, fontStyle:"italic" }}>"This is a comedy roast. All statements are hyperbolic, satirical, and for entertainment purposes only. Nothing stated here is intended as factual reporting. All views expressed are exaggerated opinions, not statements of fact."</p>
            </div>
            <div style={{ background:"rgba(201,168,76,0.05)", border:"1px solid rgba(201,168,76,0.18)", borderRadius:"6px", padding:"0.9rem 1.1rem", margin:"1rem 0" }}>
              <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.58rem", color:"#C9A84C", letterSpacing:"0.08em", marginBottom:"0.5rem" }}>SHORT VERSION (captions, cards)</div>
              <p style={{ fontFamily:"'Crimson Pro',serif", fontSize:"0.95rem", color:"#EDE8D8", lineHeight:1.6, fontStyle:"italic" }}>"Comedy roast. All statements are satirical and for entertainment only. Not factual reporting."</p>
            </div>
            ...
            {/* Back matter */}
            <div style={{ textAlign:"center", padding:"3rem 0 1rem", marginTop:"1rem", borderTop:"1px solid rgba(237,232,216,0.05)" }}>
              <div style={{ fontSize:"1.3rem", marginBottom:"0.9rem" }}>🎭 ⚖️</div>
              <p style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", color:"#444", fontSize:"0.88rem", lineHeight:1.75, maxWidth:"360px", margin:"0 auto 1.2rem" }}>
              ...
              <p style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", color:"#333", fontSize:"0.85rem", lineHeight:1.7, maxWidth:"360px", margin:"0 auto" }}>
